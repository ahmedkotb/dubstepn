...
class ApplicationController < ActionController::Base
  ...
  private
    ...
    def get_layout_fields
      ...
      num_recent_posts = 5
      if is_logged_in
        @recent_posts = if home_tag then home_tag.posts.order("sort_id DESC").limit(num_recent_posts) else [] end
        @sidebar_posts = if sidebar_tag then sidebar_tag.posts.order("sort_id DESC") else [] end
      else
        @recent_posts = if home_tag then home_tag.posts.where(:is_public => true).order("sort_id DESC").limit(num_recent_posts) else [] end
